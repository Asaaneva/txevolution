...
export const AdminLogin = () => {
  // 1. Nos conectamos al contexto global para inyectar la sesión en memoria
  const { login } = useContext(AuthContext); // 👈 Extraemos la función login
  
  // 🌟 INCORPORACIÓN REAL: Le pasamos 'login' al hook como parámetro
  const {
    formData, errors, showPassword, isSubmitting, focusedField, attemptsLeft, errorMessage, shakeField,
    setShowPassword, setFocusedField, handleChange, handleSubmit
  } = useAdminAuth(login); // Le pasamos la función login al hook
        ...
        <div className="w-full max-w-md mx-auto px-4 box-border">
          <Card>
            <div className="flex flex-col items-center w-full p-2">
              ...
              {/* El envío del formulario delega el control total al handleSubmit del hook */}
              <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6">
                ...
                <Input
                  ...
                  helperText={
                    <div className="text-[11px] leading-relaxed text-slate-500 text-left">
                      ...
                      <p className="mb-1">
                        <span className="text-green-600 font-bold">✔️ Acepta:</span>{" "}
                        <code className="bg-slate-100 px-1 py-0.5 rounded text-green-700 font-mono font-bold">
                          {".,*@#$%^&+="}
                        </code>
                      </p>
                      <p>
                        <span className="text-red-500 font-bold">❌ Prohibidos:</span>{" "}
                ...
                <div className="flex flex-col gap-4 w-full mt-2">
                  ...
                  <Button type="submit" disabled={isSubmitting || attemptsLeft <= 0}>
                ...
                <div className="flex justify-center gap-8 text-xs font-normal text-gray-500 pt-3 text-center w-full border-t border-slate-100 mt-2">
                  <a href="#forgot" className="hover:text-slate-900 hover:underline transition-colors">
                    ¿Olvidó su contraseña?
                  </a>
                  <a href="#register" className="hover:text-slate-900 hover:underline transition-colors">
                    Regístrate
                  </a>
                </div>
