...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        user = user_response.user if hasattr(
            user_response, 'user') else user_response
...
async def role_admin_required(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        result = supabase.table("profiles").select("role").eq(
            "id", current_user.id).single().execute()
        ...
        if not profile or profile.get("role") != "ADMIN":
            print(
                f"DEBUG: Acceso denegado para el ID {current_user.id}. Rol encontrado: {profile.get('role') if profile else 'None'}")
            raise HTTPException(
